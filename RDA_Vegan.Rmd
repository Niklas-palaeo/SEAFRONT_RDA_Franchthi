...
Fauna <- bind_rows(Fauna_FAN,Fauna_FAS,Fauna_Paralia) %>% 
  ...
  mutate(caprines=caprines+caprine) %>% 
  select(-caprine)

rm(Fauna_FAN,Fauna_FAS,Fauna_Paralia,Molluscs)


saveRDS(Fauna,file = here("data","pre-formatted dataframes","Fauna.rds"))
...
select(-c(culm_nodes_indet_frags, glume_base_indet, 
            indet_frag, rachis_frag_indet, seed_indet, spikelet_fork_indet_frags,sp_no_1))



saveRDS(Bot,file = here("data","pre-formatted dataframes","Bot.rds"))
...
## Filtering Data
Here we combine the d18O data with the faunal data. We do not use the more detailed climatic data that calculates the seasonal range or variability, because that leads to many NAs, which reduce the already small number of layers for the RDA analysis. This information is added again in the scatterplots though. 
### Filtered Fauna
```{r}
...
saveRDS(Filtered_Fauna,file = here("data","pre-formatted dataframes","Filtered_Fauna.rds"))
...
saveRDS(Filtered_D18O_Fauna,file = here("data","pre-formatted dataframes","Filtered_D18O_Fauna.rds"))
...
saveRDS(Filtered_Bot,file = here("data","pre-formatted dataframes","Filtered_Bot.rds"))
...
saveRDS(Filtered_D18O_Bot,file = here("data","pre-formatted dataframes","Filtered_D18O_Bot.rds"))
...
saveRDS(Joined_D18O_Fauna,file = here("data","pre-formatted dataframes","Joined_D18O_Fauna.rds"))
...
saveRDS(Corrs_Fauna,file = here("data","pre-formatted dataframes","Corrs_Fauna.rds"))
...
saveRDS(Joined_D18O_Bot,file = here("data","pre-formatted dataframes","Joined_D18O_Bot.rds"))
...
saveRDS(Corrs_Bot,file = here("data","pre-formatted dataframes","Corrs_Bot.rds"))
...
```{r}
saveRDS(Corrs_Fauna,here("data","pre-formatted dataframes","Corrs_Fauna.rds"))
saveRDS(Corrs_Fauna,here("data","pre-formatted dataframes","Fauna.rds"))
saveRDS(Corrs_Fauna,here("data","pre-formatted dataframes","Filtered_D18O_Fauna.rds"))
saveRDS(Corrs_Fauna,here("data","pre-formatted dataframes","Joined_D18O_Fauna.rds"))
```
